 ...
 */
const validateAndCalculateDiscount = async (params) => {
    ...
    // 1. Fetch Coupon
    const coupon = await coupon_model_1.default.findOne({ code: code.toUpperCase() }).session(session || null);
    if (!coupon) {
    ...
    if (isNew) {
        ...
        if (coupon.perUserLimit > 0) {
            const userUsageCount = await order_model_1.default.countDocuments({
                userId,
        ...
        if (coupon.isForNewUserOnly) {
            const userOrderCount = await order_model_1.default.countDocuments({
                userId,
                order_status: { $ne: "cancelled" }
            }).session(session || null);
            if (userOrderCount > 0) {
    ...
    const productIds = cartItems.map(item => new mongoose_1.default.Types.ObjectId(item.productId.toString()));
